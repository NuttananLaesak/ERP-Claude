...
export default async function EmployeeDetailPage({
  ...
  const [employee, departments, positions] = await Promise.all([getEmployee(id), getDepartments(), getPositions()]);
      ...
      <div className="relative mx-auto max-w-3xl">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-3">
                    <span className="text-xs text-white/35">{employee.employeeId}</span>
              ...
              <div className="flex items-center gap-2">
                {employee.status !== "ACTIVE" && (
                  <form action={updateEmployeeStatus.bind(null, employee.id, "ACTIVE")}>
                    <button type="submit" className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-400 hover:bg-emerald-500/20">
                      Set Active
                    </button>
                  </form>
                )}
                {employee.status !== "ON_LEAVE" && (
                  <form action={updateEmployeeStatus.bind(null, employee.id, "ON_LEAVE")}>
                    <button type="submit" className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-400 hover:bg-amber-500/20">
                      Set Leave
                    </button>
                  </form>
                )}
                {employee.status !== "INACTIVE" && (
                  <form action={updateEmployeeStatus.bind(null, employee.id, "INACTIVE")}>
                    <button type="submit" className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-white/40 hover:bg-white/[0.07]">
                      Deactivate
                    </button>
                  </form>
                )}
                <form action={deleteEmployee.bind(null, employee.id)}>
                  <button type="submit" className="rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-500/20">
                    Delete
                  </button>
                </form>
              </div>
        ...
        <AnimatedSection delay={0.09}>
          <div className="mb-6 rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5">
            <p className="mb-3 text-xs font-medium uppercase tracking-widest text-white/25">Portal Access</p>
            {employee.userId ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-white/30">Employee can log in with their email</span>
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-xs text-white/40">No login account yet. Default password will be the employee ID.</p>
        ...
        <AnimatedSection delay={0.12}>
          ...
          <EditEmployeeForm employee={employee} departments={departments} positions={positions} />
        </AnimatedSection>
      </div>
    </div>
  );
