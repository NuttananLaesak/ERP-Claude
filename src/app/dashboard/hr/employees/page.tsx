        ...
        <AnimatedSection delay={0}>
          <div className="mb-8 flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-white/35">{employees.length} total</p>
            </div>
            <Link
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/[0.06]">
                      {["Employee", "Department", "Position", "Status", "Hire Date", ""].map((h) => (
                        <th key={h} className="px-6 py-3 text-left text-xs font-medium text-white/35">{h}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {employees.map((emp) => (
                      <tr key={emp.id} className="group transition-colors hover:bg-white/[0.02]">
                        <td className="px-6 py-4">
                          <Link href={`/dashboard/hr/employees/${emp.id}`} className="flex items-center gap-3 group/row">
                            ...
                            <div>
                              <p className="text-sm font-medium text-white/80 group-hover/row:text-white/95 transition-colors">{emp.name}</p>
                              <p className="text-xs text-white/35">{emp.email}</p>
                            </div>
                          </Link>
                        </td>
                        <td className="px-6 py-4 text-sm text-white/55">{emp.department.name}</td>
                        <td className="px-6 py-4 text-sm text-white/55">{emp.position?.name ?? "—"}</td>
                        <td className="px-6 py-4">
                          <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[emp.status]}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                            {emp.status !== "ACTIVE" && (
                              <form action={updateEmployeeStatus.bind(null, emp.id, "ACTIVE")}>
                                <button type="submit" className="rounded-lg px-2 py-1 text-xs text-emerald-400 hover:bg-emerald-500/10">
                                  Activate
                                </button>
                              </form>
                            )}
                            {emp.status !== "ON_LEAVE" && (
                              <form action={updateEmployeeStatus.bind(null, emp.id, "ON_LEAVE")}>
                                <button type="submit" className="rounded-lg px-2 py-1 text-xs text-amber-400 hover:bg-amber-500/10">
                                  Leave
                                </button>
                              </form>
                            )}
                            <form action={deleteEmployee.bind(null, emp.id)}>
                              <button type="submit" className="rounded-lg px-2 py-1 text-xs text-red-400 hover:bg-red-500/10">
                                Delete
                              </button>
                            </form>
                          </div>
