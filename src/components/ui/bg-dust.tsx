const meteors = [
  { top: "6%",  left: "70%", delay: "0s",   dur: "3s"   },
  { top: "20%", left: "88%", delay: "1.8s", dur: "2.8s" },
  { top: "3%",  left: "52%", delay: "3.2s", dur: "3.5s" },
  { top: "40%", left: "94%", delay: "0.9s", dur: "2.5s" },
  { top: "58%", left: "78%", delay: "4.1s", dur: "3.2s" },
];
...
const dustColors: Record<number, string> = {
  0: "rgba(167,139,250,.55)",
  1: "rgba(103,232,249,.55)",
  2: "rgba(232,121,249,.5)",
  3: "rgba(52,211,153,.5)",
  4: "rgba(251,191,36,.45)",
  5: "rgba(251,113,133,.5)",
  6: "rgba(56,189,248,.5)",
  7: "rgba(129,140,248,.55)",
};

const dustGlow: Record<number, string> = {
  0: "rgba(167,139,250,.4)",
  1: "rgba(103,232,249,.4)",
  2: "rgba(232,121,249,.35)",
  3: "rgba(52,211,153,.35)",
  4: "rgba(251,191,36,.3)",
  5: "rgba(251,113,133,.35)",
  6: "rgba(56,189,248,.35)",
  7: "rgba(129,140,248,.4)",
};

export function BgDust() {
  return (
    <div className="pointer-events-none fixed inset-0 overflow-hidden">
      {/* Orbs */}
      <div
        className="absolute -top-[30%] -left-[15%] h-[85vh] w-[70vw] rounded-full bg-violet-700/[0.12] blur-[150px]"
        style={{ animation: "orb-a 12s ease-in-out infinite" }}
      />
      <div
        className="absolute -bottom-[25%] -right-[15%] h-[75vh] w-[65vw] rounded-full bg-cyan-500/[0.08] blur-[160px]"
        style={{ animation: "orb-b 15s ease-in-out infinite" }}
      />
      <div
        className="absolute top-[30%] right-[-8%] h-[50vh] w-[42vw] rounded-full bg-fuchsia-600/[0.07] blur-[120px]"
        style={{ animation: "orb-c 18s ease-in-out infinite" }}
      />
      <div
        className="absolute bottom-[10%] left-[-5%] h-[40vh] w-[38vw] rounded-full bg-indigo-500/[0.07] blur-[110px]"
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 72% 72% at 50% 50%, transparent 25%, #030309 100%)",
        }}
      />
        ...
        >
          <div
            style={{
              ...
              height: "1px",
              background:
                "linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(167,139,250,.7) 50%, rgba(6,182,212,.5) 80%, rgba(255,255,255,0) 100%)",
              animation: `meteor ${m.dur} ease-in ${m.delay} infinite`,
              animationFillMode: "both",
              boxShadow: "0 0 4px rgba(167,139,250,.4)",
            }}
          />
        </div>
      ))}
